...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const currentEmployee = employees.find((emp) => emp.name === userInfo.username);

    const presentDays = attendance.filter(
      (att) => att.userId === currentEmployee?.id && att.status === "present"
    ).length;

    setAttendanceCount(presentDays);
    setProfileData({ ...userInfo, id: currentEmployee?.id });
  }, []);
