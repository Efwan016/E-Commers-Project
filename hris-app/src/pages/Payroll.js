...
const Payroll = () => {
    ...
    useEffect(() => {
        ...
        const newPayrolls = storedEmployees
            .map((emp) => {
    ...
    const handleEditStatus = (id, status) => {
        const updated = payrolls.map((p) => (p.id === id ? { ...p, status } : p));
        saveToStorage(updated);
    };

    const handleDelete = (id) => {
        if (role !== "admin") {
            alert("Anda tidak punya akses menghapus data!");
            return;
        }
    ...
    const handleEdit = (id) => {
        if (role !== "admin") {
            alert("Anda tidak punya akses mengedit data!");
            return;
        }
        ...
        <div className="container mt-4 payroll-container">
            ...
            <div className="mb-3 d-flex justify-content-between align-items-center">
                ...
                <div className="d-flex gap-2">
                    ...
                    {role === "admin" && (
                        <button className="btn btn-primary" onClick={() => navigate("/payroll/new")}>
                            ➕ Add Payroll
                        </button>
                    )}
                </div>
                    ...
                    <tbody>
                        ...
                        {filtered.map((p) => (
                            <tr key={p.id}>
                                ...
                                <td>
                                    <span
                                        className={`badge bg-${p.status === "Paid" ? "success" : "warning"}`}
                                    >
                                        {p.status}
                                    </span>
                                </td>
